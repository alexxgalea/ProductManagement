...
class GoodsReceipt(models.Model):
    ...
    location = models.ForeignKey(
        "accounts.Location", on_delete=models.PROTECT, related_name="goods_receipts"
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["supplier", "document_number"], name="uq_supplier_document_number"
            )
        ]
...
class GoodsReceiptLine(models.Model):
    ...
    ingredient = models.ForeignKey(
        "core.Ingredient", on_delete=models.PROTECT, related_name="lines"
    )
    quantity = models.DecimalField(
        max_digits=12,
        decimal_places=3,
        validators=[MinValueValidator(0, "Cantitatea de pe bon nu poate fi negativa")],
    )
    unit_price = models.DecimalField(
        max_digits=12,
        decimal_places=3,
        validators=[MinValueValidator(0, "Pretul unitar nu poate fi negativ")],
    )
