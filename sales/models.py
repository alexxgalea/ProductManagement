...
class Receipt(models.Model):
    location = models.ForeignKey(
        "accounts.Location", on_delete=models.PROTECT, related_name="receipts"
    )
...
class ReceiptLineQuerySet(models.QuerySet):
    def top_products(self, location=None, start=None, end=None, limit=10):
        ...
        return (
            qs.values("menu_item", "menu_item__name")
            .annotate(
                qty=Sum("quantity"),
                revenue=Sum(F("quantity") * F("unit_price"), output_field=models.DecimalField()),
            )
            .order_by("-qty")[:limit]
        )
