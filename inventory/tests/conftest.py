...
def create_stock_count_line(create_stock_count, create_ingredient):
    return StockCountLineFactory.create(stock_count=create_stock_count, ingredient=create_ingredient)
...
def create_reported_loss(create_stock):
    stock = create_stock
    return LossFactory.create(location=stock.location, ingredient=stock.ingredient, quantity=Decimal("5.00"), reason=ReportedLoss.Reason.ROTTEN)
