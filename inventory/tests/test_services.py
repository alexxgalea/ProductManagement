...
def test_repeated_apply_goods_receipt_same_effect(
    create_goods_receipt, create_goods_receipt_line, create_stock, times
):
...
@pytest.mark.django_db(transaction=True)
def test_two_workers_racing_do_not_double_apply(
    create_goods_receipt, create_goods_receipt_line, create_stock
):
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.parametrize("times", [1, 2])
def test_repeated_apply_stock_count_same_effect(
    create_stock, create_stock_count, create_stock_count_line, times
):
    for _ in range(times):
