...
class ReportedLoss(models.Model):
    ...
    ingredient = models.ForeignKey(
        "core.Ingredient", on_delete=models.PROTECT, related_name="reported_losses"
    )
    quantity = models.DecimalField(
        max_digits=12,
        decimal_places=3,
        validators=[MinValueValidator(0.0, message="Cantitatea nu poate fi negativa")],
    )
...
class StaffConsumptionBudget(models.Model):
    location = models.ForeignKey(
        "accounts.Location",
        related_name="consumption_budgets",
        on_delete=models.PROTECT,
    )
    month = models.DateField(
        verbose_name="Luna si Anul",
        help_text="Alege orice zi; sistemul va salva prima zi a lunii",
    )
    amount = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        validators=[MinValueValidator(0, message="Valoarea nu poate fi negativa")],
    )
