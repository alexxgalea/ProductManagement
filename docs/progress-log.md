...
| 59 | `defaults={"quantity", 0}` (set, virgulă) în `get_or_create` | `defaults` vrea un **dict** (`{"quantity": 0}`, cu `:`), nu un set. A trecut testul din întâmplare (rândul exista deja din `setUp`, deci ramura `defaults` nu s-a atins) — „merge" ≠ „corect". |
| 60 | Test scris cu valoarea așteptată greșită (`20−5` aștepta `5`) + `Reason.STRICAT` | Verifică mereu că **numărul așteptat e corect matematic** (20−5=**15**), altfel testezi împotriva unei valori greșite. Enum: accesezi membrul pe **numele** lui (`Reason.rotten`), nu pe **valoarea** DB (`"STRICAT"`). |
| 61 | La `apply_stock_count` foloseam `sc.lines` (nu există) | Reverse accessor = **`related_name`-ul real** (`StockCountLine` are `related_name="stock_lines"` → `sc.stock_lines`, nu `sc.lines`). Cod service ↔ model trebuie să se potrivească. Mișcări de stoc: **delta** (NIR/pierdere, `F()+/−`, flag OBLIGATORIU) vs **setare absolută** (numărătoare, `= counted_quantity`, natural idempotentă dar flag util ca să nu **suprascrie** mișcări ulterioare + audit). |
